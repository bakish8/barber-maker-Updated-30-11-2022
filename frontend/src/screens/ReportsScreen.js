...
const ReportsScreen = ({ history }) => {
  ...
  const DAILY_REPORTS_LIST = useSelector((state) => state.DAILY_REPORTS_LIST)
  const { result_list_daily_Reports } = DAILY_REPORTS_LIST

  const Weekly_REPORTS_LIST = useSelector((state) => state.Weekly_REPORTS_LIST)
  const { result_list_WEEKLY_Reports } = Weekly_REPORTS_LIST

  const MONTHLY_REPORTS_LIST = useSelector(
    (state) => state.MONTHLY_REPORTS_LIST
  )
  const { resultReportsMonth } = MONTHLY_REPORTS_LIST
  ...
  const [DisplayDailyReports, setDisplayDailyReports] = useState(false)
  const [DisplayMonthlyReports, setDisplayMonthlyReports] = useState(false)
  const [DisplayWeeklyReports, setDisplayWeeklyReports] = useState(false)
  ...
  const DisplayDailyReportsFunction = () => {
    setDisplayDailyReports(true)
    setDisplayMonthlyReports(false)
    setDisplayWeeklyReports(false)
  }
  const DisplayWeeklyReportsFunction = () => {
    setDisplayWeeklyReports(true)
    setDisplayMonthlyReports(false)
    setDisplayDailyReports(false)
  }
  const DisplayMonthlyReportsFunction = () => {
    setDisplayMonthlyReports(true)
    setDisplayWeeklyReports(false)
    setDisplayDailyReports(false)
  }

  const CreateSicumShowTable = () => {
  ...
  const SearchReportShowMenuFunction = () => {
  ...
  const WatchReportShowMenuFunction = () => {
  ...
  useEffect(() => {
    if (userInfo && userInfo.isAdmin) {
      ...
      dispatch(listDailyReports(userInfo._id))
      dispatch(listWeeklyReports(userInfo._id))
      dispatch(listMonthlyReports(userInfo._id))
    } else {
      history.push('/login')
    }
  }, [dispatch, history, userInfo, workingDayInfo])
  //USE EFFECT  FOR RECIPT CREATING-IF PAST? - STATUS
  useEffect(() => {
    if (oneworkingdays && oneworkingdays[0]) {
      const Calculate_Month = oneworkingdays[0].date.substring(0, 5)
      const month_Absulute = Calculate_Month.slice(-2)
      const Year_Absulute = oneworkingdays[0].date.slice(-4)
      const Day_Absulute = oneworkingdays[0].date.substring(0, 2)
      if (
        year === Year_Absulute &&
  ...
  //Swals for saing report in system
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <>
          <div id='centerme'>
            <h1 className='whiteme'>דוחו"ת וסיכומים</h1>
          </div>
                  ...
                  <Table bordered className='whiteme' id='tablewhite'>
                    ...
                    <tbody id='centertext'>
                      {workingdays.length === 0 ? (
                        <Message>
                          {' '}
                          אין תורים במערכת לחץ על התאריך המבוקש ביומן העבודה על
                          מנת ליצור יום עבודה
                        </Message>
                      ) : (
                        workingdays
          ...
          {DisplayMonthlyReports && (
            <Col md={12}>
              <div>
                {resultReportsMonth && resultReportsMonth[0] ? (
                  resultReportsMonth.map((report) => (
                    <LinkContainer to={`/admin/reports/${report._id}`}>
                      <div id='reportBTNdiv' className='scaleAbit'>
                        <Button id='REPORTbtn' key={report._id}>
                          <div id='clcktime'>
                            {' '}
                            <h4 id='ReportbtnstextBIG'>
                              {' '}
                              סיכום חודשי
                              <br />
                            </h4>
                            <h7 ID='Reportbtnstextsmall'>
                              לתאריכים {report.date}
                            </h7>
                          </div>
                        </Button>
                      </div>
                  ...
                  <div id='weSorryAllTafus'>אין לך עדיין דוחו"ת חודשיים </div>
                )}
              </div>
            </Col>
          )}
          {DisplayWeeklyReports && (
            <Col md={12}>
              <div>
                {result_list_WEEKLY_Reports && result_list_WEEKLY_Reports[0] ? (
                  result_list_WEEKLY_Reports.map((report) => (
                    <LinkContainer to={`/admin/reports/${report._id}`}>
                      <div id='reportBTNdiv' className='scaleAbit'>
                        <Button id='REPORTbtn' key={report._id}>
                          <div id='clcktime'>
                  ...
                  <div id='weSorryAllTafus'>אין לך עדיין דוחו"ת שבועיים </div>
                )}
              </div>
            </Col>
          )}
          {DisplayDailyReports && (
            <Col md={12}>
              <div>
                {result_list_daily_Reports && result_list_daily_Reports[0] ? (
                  result_list_daily_Reports.map((report) => (
                    <LinkContainer to={`/admin/reports/${report._id}`}>
                      <div id='reportBTNdiv' className='scaleAbit'>
                        <Button id='REPORTbtn' key={report._id}>
                          <div id='clcktime'>
                  ...
                  <div id='weSorryAllTafus'>אין לך עדיין סיכומים יומיים </div>
                )}
              </div>
            </Col>
          )}
