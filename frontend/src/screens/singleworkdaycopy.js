...
const SingleWorkDayScreen = ({ history, match }) => {
  ...
  const submitHandler = (e) => {
    ...
    console.log(time)
    console.log(sapar)
    swal('מוסיף תור למערכת אנא המתן', {
      buttons: false,
      timer: 10000,
    })
  ...
  const submitHandler2 = (e) => {
    ...
    dispatch(createClocks(WorkDayid, time, time2, sapar)).then(
      swal('מוסיף תורים למערכת אנא המתן', {
        buttons: false,
        timer: 20000,
      })
    )
            ...
            <div>
              ...
              <div id='centerme'>
                <Button
                  ...
                  className='my-3'
                >
                  <i className='fas fa-plus'></i> הוספה מהירה
                </Button>
                <br />
                <Button
                  onClick={showFormNow}
              ...
              <Table bordered hover responsive id='tablewhite'>
                ...
                <tbody id='centertext'>
                  {clockList
                    ...
                    .sort((a, b) => {
                      const TimeA = new Date(` ${a.time}`).valueOf()
                      const TimeB = new Date(` ${b.time}`).valueOf()
                      if (TimeA > TimeB) {
