...
const PickDate = ({ location, history, match }) => {
  ...
  const convert = (date, format = state.format) => {
    ...
    WorkingDaySubmitHandler(dateData, day, sid)
  }

  const WorkingDaySubmitHandler = (dateData, day, sid) => {
    if (!dateData) {
      setMessage('תאריך לא תקין')
    } else {
      dispatch(PICKWorkingDay(dateData, day, sid)) //*** */
  ...
  useEffect(() => {
    if (foundWorkdayInfo && FINDSuccses === true && userInfo) {
      history.push(
        `/business/${match.params.id}/picktipul/${foundWorkdayInfo._id}`
      ) //pick tipul
